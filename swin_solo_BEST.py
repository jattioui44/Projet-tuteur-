"""
Swin Transformer Tiny SEUL - Configuration OPTIMALE
...
class Config:
    ...
    # Sauvegarde
    BEST_MODEL_PATH = "./best_swin_solo.pth"
...
class SwinClassifier(nn.Module):
    """Swin Transformer Tiny avec classifier simple et efficace"""
...
def main():
    print("\n" + "="*80)
    print("🚀 SWIN TRANSFORMER TINY SOLO - CONFIGURATION OPTIMALE")
    print("="*80)
    ...
    # Modèle
    print("\n🔨 Construction modèle Swin Transformer...")
    model = SwinClassifier(num_classes=Config.NUM_CLASSES, dropout=Config.DROPOUT)
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        if val_loss < best_val_loss:  # ← Changement: < au lieu de >
    ...
    plt.title(f'Swin Solo (Acc: {test_acc:.1f}%, Kappa: {kappa:.3f})')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig('confusion_matrix_swin_solo.png', dpi=300, bbox_inches='tight')
    print(f"\n✅ Confusion matrix: confusion_matrix_swin_solo.png")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('Accuracy (Swin Solo)')
    plt.legend()
    plt.grid(True)
    
    plt.tight_layout()
    plt.savefig('training_curves_swin_solo.png', dpi=300, bbox_inches='tight')
    print(f"✅ Training curves: training_curves_swin_solo.png")
