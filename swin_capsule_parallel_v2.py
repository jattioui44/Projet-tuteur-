...
class Config:
    ...
    # Sauvegarde
    BEST_MODEL_PATH = "./best_parallel_improved.pth"
...
class ParallelSwinCapsule(nn.Module):
    ...
    def __init__(self, num_classes=5, pretrained=True):
        ...
        # ===== ÉGALISATION DES BRANCHES =====
        print("  ⚖️  Égalisation des branches à 256 features chacune")
        
        # Projection Swin: 768 → 256
        ...
        # Projection Capsules: 80 → 256
        capsule_flat_dim = num_classes * Config.ROUTING_CAP_DIM
        self.capsule_projection = nn.Sequential(
            nn.Linear(capsule_flat_dim, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(0.3)
        )
        
        print(f"    Swin: {self.swin_dim} → 256")
        print(f"    Capsules: {capsule_flat_dim} → 256")
        
        # ===== FUSION =====
        print("  🔀 Fusion des deux branches (256 + 256 = 512)")
        
        fusion_input_dim = 256 + 256  # Égalité parfaite!
        ...
        print(f"    Fusion: 512 → 256 → 128 → 32 → {num_classes}")
    ...
    def forward(self, x):
        ...
        # ÉGALISATION: Projeter les deux branches à 256
        swin_projected = self.swin_projection(swin_features)  # [batch, 256]
        capsule_flat = capsule_vectors.view(capsule_vectors.size(0), -1)  # [batch, num_classes * cap_dim]
        capsule_projected = self.capsule_projection(capsule_flat)  # [batch, 256]
        
        # FUSION: 256 + 256 = 512
        fusion_input = torch.cat([swin_projected, capsule_projected], dim=1)  # [batch, 512]
...
def main():
    ...
    plt.title('Matrice de Confusion (Parallel Improved)')
    plt.ylabel('Vraie')
    plt.xlabel('Prédite')
    plt.savefig('confusion_matrix_parallel_improved_v2.png', dpi=300, bbox_inches='tight')
    ...
    plt.xlabel('Époque')
    plt.ylabel('Loss')
    plt.title('Loss (Parallel Improved)')
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel('Époque')
    plt.ylabel('Accuracy (%)')
    plt.title('Accuracy (Parallel Improved)')
    plt.legend()
    plt.grid(True)
    
    plt.tight_layout()
    plt.savefig('training_curves_parallel_improved_v2.png', dpi=300, bbox_inches='tight')
