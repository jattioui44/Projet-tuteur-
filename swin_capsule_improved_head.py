"""
Architecture Hybride Swin + Capsule - VERSION IMPROVED HEAD
CODE IDENTIQUE à no_weights v2 SAUF:
- Tête de classification améliorée avec couche intermédiaire 32 neurones
- Architecture: Features → [Dense 512] → [Dense 32] → [Dense 5]
- Plus de capacité d'apprentissage dans la classification finale
"""
...
class Config:
    ...
    USE_CLASS_WEIGHTS = False  # ← DÉSACTIVÉ pour test
    
    # Focal Loss - NOUVEAU! ⭐
    USE_FOCAL_LOSS = False  # ← DÉSACTIVÉ pour test
    FOCAL_ALPHA = 0.25
    FOCAL_GAMMA = 2.0
    
    # Sauvegarde
    BEST_MODEL_PATH = "./best_capsule_improved_head.pth"
...
class SwinCapsuleHybrid(nn.Module):
    def __init__(self, num_classes=5, pretrained=True):
        ...
        # Swin classifier amélioré avec couche intermédiaire 32
        self.swin_classifier = nn.Sequential(
            nn.Linear(swin_feature_dim, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 32),  # Couche intermédiaire 32 neurones
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(32, num_classes)  # Sortie finale 5 classes
        )
        
        fusion_input_dim = swin_feature_dim + (num_classes * Config.ROUTING_CAP_DIM)
        
        # Fusion layer améliorée avec architecture plus profonde
        self.fusion_layer = nn.Sequential(
            ...
            # Couche intermédiaire 32 neurones (NOUVELLE!)
            nn.Linear(Config.FUSION_DIM // 2, 32),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.Dropout(0.3),
...
def main():
    print("\n" + "="*70)
    print("🚀 SWIN + CAPSULE - VERSION IMPROVED HEAD")
    print("="*70)
    print("AMÉLIORATIONS:")
    print("❌ Class Weights: DÉSACTIVÉS")
    print("❌ Focal Loss: DÉSACTIVÉ")
    print("✅ Architecture: Swin + Capsules")
    print("✅ NOUVELLE TÊTE: Features → 256 → 32 → 5 ⭐")
    print("✅ BatchNorm après chaque couche")
    print("✅ Plus de capacité pour apprendre")
    print("="*70)
    ...
    plt.ylabel('Vraie')
    plt.xlabel('Prédite')
    plt.savefig('confusion_matrix_improved_head.png', dpi=300, bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig('training_curves_improved_head.png', dpi=300, bbox_inches='tight')
